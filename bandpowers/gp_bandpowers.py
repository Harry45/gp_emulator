...
Description:
Gaussian Process script for emulating the KiDS-450 band powers
...
filename = "logs/gaussian_process.log"


class GP:

    def __init__(self, data, sigma=[-5.0], train=False, nrestart=5, ndim=6):
        ...
        # our data - a numpy array
        data = np.array(data)

        # the inputs, we call it \theta
        self.theta = data[:, 0:-1]

        # the output - a vector of size N
        self.y_ = data[:, -1]
        ...
        # rescale the output such that it has
        ...
        # dimension of the problem via theta
        self.d = self.theta.shape[1]

        # input dimension
        self.ndim = ndim

        assert self.d == self.ndim, 'Mis-match input dimensions'

        # kernel amplitude
        self.width = np.zeros(1)

        # characteristic lengthscales
        self.scale = np.zeros(self.ndim)

        # add information to logger
        self.logger.info(
            'The number of training points is {}'.format(self.N))
        self.logger.info(
            'The dimension of the problem is  {}'.format(self.ndim))
    ...
    def grad_log_like(self, theta):
        ...
        for i in range(self.ndim):
            dist_ = self.distancePerdim(self.theta_[:, i], self.theta_[:, i])
    ...
    def sampleBandPower(self, testPoint, mean=True, nsamples=200):
        '''
        Function to return samples from the Gaussian Process

        This is important if we want to marginalise over the GP
        uncertainty numerically.

        Args:
            (array) testPoint: an array of size ndim
        ...
        # compute mean
        if mean:
            mu = self.prediction(testPoint, returnVar=False)
            return mu

        # or samples
        else:
            mu, var = self.prediction(testPoint, returnVar=True)
